...
from .bem_plot import draw_placeholder as draw_bem_placeholder, draw_polar_plot
...
class AthConfigStudio(tk.Tk):
    ...
    def _build_shell(self) -> None:
        ...
        for tab_name in ("Global", "Geometry", "Morph", "Mesh", "Simulation", "Output", "Preview", "BEM", "Advanced"):
    ...
    def _build_status_panel(self, parent: ttk.Frame) -> None:
        """Build a top-level runtime status card for preview and group diagnostics."""
        card = ttk.LabelFrame(parent, text="執行狀態 / 預覽鏈路", style="Card.TLabelframe", padding=14)
        card.grid(row=2, column=0, sticky="ew")
        for column in range(4):
            card.columnconfigure(column, weight=1 if column % 2 == 1 else 0)

        labels = [
            ("建置識別", self.build_var),
            ("專案根目錄", self.project_root_var),
            ("Python 執行環境", self.python_var),
            ("目前號角檔", self.current_cfg_var),
            ("推定輸出目錄", self.output_dir_var),
            ("預覽檔案", self.preview_path_var),
            ("預覽狀態", self.preview_status_var),
            ("BEM 網格", self.mesh_status_var),
            ("分群狀態", self.group_status_var),
        ]

        for index, (label, variable) in enumerate(labels):
            row = index // 2
            column = (index % 2) * 2
            ttk.Label(card, text=f"{label}：", style="CardLabel.TLabel").grid(
                row=row,
                column=column,
                sticky="nw",
                padx=(0, 10),
            ...
            ttk.Label(
                ...
                justify="left",
                wraplength=440,
            ).grid(row=row, column=column + 1, sticky="ew", pady=(0, 8))
    ...
    def _build_bem_panel(self, target: ttk.Frame) -> None:
        ...
        mesh_card.grid(row=1, column=0, sticky="nsew", padx=14, pady=(14, 0))
        mesh_card.columnconfigure(0, weight=1)
        self.bem_mesh_text = self._make_dark_text(mesh_card, height=11, wrap="word")
        ...
        summary_card.grid(row=2, column=0, sticky="nsew", padx=14, pady=(14, 0))
        summary_card.columnconfigure(0, weight=1)
        self.bem_summary_text = self._make_dark_text(summary_card, height=12, wrap="word")
        ...
        plot_card.grid(row=3, column=0, sticky="nsew", padx=14, pady=(14, 18))
        plot_card.columnconfigure(0, weight=1)
        ttk.Label(plot_card, textvariable=self.bem_plot_caption_var, style="Hint.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 10))
        ...
        self.bem_polar_canvas.grid(row=1, column=0, sticky="nsew")
    ...
    def inspect_bem_mesh(self) -> None:
        ...
        self._update_runtime_status()
        self.notebook.select(self.notebook.tabs()[list(self.tab_bodies.keys()).index("BEM")])
    ...
    def _refresh_bem_plot(self) -> None:
        caption = draw_polar_plot(self.bem_polar_canvas, self.bem_polar_rows)
        if caption:

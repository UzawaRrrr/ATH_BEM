...
def derive_auto_enclosure(
    ...
    xy_extra_total: float = 10.0,
    extra_depth: float = 10.0,
    per_side: bool = True,
    edge_radius_mm: float = 10.0,
    front_resolution: float = 25.0,
    back_resolution: float = 25.0,
) -> dict[str, object]:
    """
    ...
    - ENCLOSURE.EdgeRadius = edge_radius_mm (clamped to side margin)
    - ENCLOSURE.FrontResolution = "q1,q2,q3,q4"
    - ENCLOSURE.BackResolution = "q1,q2,q3,q4"
    """
    ...
    edge_radius = max(min(float(edge_radius_mm), side_margin), 0.0)
    side_text = _fmt(side_margin)
    front_text = _fmt(max(float(front_resolution), 0.0))
    back_text = _fmt(max(float(back_resolution), 0.0))
    ...
    result["ENCLOSURE.FrontResolution"] = f"{front_text},{front_text},{front_text},{front_text}"
    result["ENCLOSURE.BackResolution"] = f"{back_text},{back_text},{back_text},{back_text}"
    return result

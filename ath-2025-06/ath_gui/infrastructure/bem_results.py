...
def format_summary_text(results: dict[str, object]) -> str:
    ...
    boundary_role = dict(summary.get("boundary_role_diagnostics", {}))
    ...
    if boundary_role:
        lines.extend(
            [
                (
                    "邊界覆蓋："
                    f"source={_as_float(boundary_role.get('source_area_m2', 0.0)):.6e} m^2 "
                    f"({_as_float(boundary_role.get('source_area_ratio', 0.0)):.1%}) | "
                    f"wall={_as_float(boundary_role.get('wall_area_m2', 0.0)):.6e} m^2 "
                    f"({_as_float(boundary_role.get('wall_area_ratio', 0.0)):.1%})"
                ),
                (
                ...
                (
                    "拓樸檢查："
                    f"開邊 {_as_int(boundary_role.get('boundary_edge_count', 0))} | "
                    f"非流形邊 {_as_int(boundary_role.get('nonmanifold_edge_count', 0))} | "
                    f"連通分量 {_as_int(boundary_role.get('connected_component_count', 0))}"
                ),
            ]
        )

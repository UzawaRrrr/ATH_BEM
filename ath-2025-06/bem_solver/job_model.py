...
from symmetry import SymmetryConfig, parse_symmetry_config, validate_symmetry_config
...
class BemJob:
    ...
    plane: str = "XZ"
    angle_range_mode: str = "full_circle"
    theta_count: int = 361
    ...
    def from_mapping(cls, payload: dict[str, object], *, job_path: Path | None = None) -> "BemJob":
        job = cls(
            ...
            angle_range_mode=str(payload.get("angle_range_mode", "full_circle")).lower(),
            ...
            symmetry=parse_symmetry_config(payload.get("symmetry") if isinstance(payload.get("symmetry"), dict) else None),
            job_path=job_path,
        )
        job.validate()
        return job
    ...
    def validate(self) -> None:
        ...
        if self.angle_range_mode not in {"full_circle", "half_circle", "quarter_circle"}:
            raise ValueError("angle_range_mode must be one of: full_circle, half_circle, quarter_circle.")
        if self.reference_pressure <= 0:
            raise ValueError("reference_pressure must be greater than zero.")
        validate_symmetry_config(self.symmetry)
